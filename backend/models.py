...
class Notification(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)  # Admin user ID
    type = db.Column(db.String(50), nullable=False)  # 'chat', 'consultation', 'booking', etc.
    title = db.Column(db.String(200), nullable=False)
    message = db.Column(db.Text, nullable=False)
    related_id = db.Column(db.Integer, nullable=True)  # ID of related object (chat_id, consultation_id, etc.)
    is_read = db.Column(db.Boolean, default=False)
