...
const ConsultStatusBadge = ({ status }) => {
  const map = {
    pending:   { color: "text-amber-600 bg-amber-50 border-amber-200",   icon: <FaHourglassHalf className="text-[10px]" />, label: "Pending"   },
    confirmed: { color: "text-green-700 bg-green-50 border-green-200",   icon: <FaCheckCircle   className="text-[10px]" />, label: "Confirmed" },
    cancelled: { color: "text-red-600   bg-red-50   border-red-200",     icon: <FaTimesCircle   className="text-[10px]" />, label: "Cancelled" },
    completed: { color: "text-stone-600 bg-stone-50 border-stone-200",   icon: <FaCheckCircle   className="text-[10px]" />, label: "Completed" },
  };
  const s = map[status] || map.pending;
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] uppercase tracking-widest px-2 py-1 border rounded-full font-medium ${s.color}`}>
      {s.icon} {s.label}
    </span>
  );
};
...
export default function AdminDashboard() {
  ...
  // Consultation status update state
  ...
  const fetchConsultations = async () => {
    ...
    try {
      ...
      console.error(e);
      setConsultsError("Failed to load consultations.");
    } finally {
    ...
    <div className="flex flex-col md:flex-row h-screen bg-[#F9F8F6] font-sans text-stone-800 overflow-hidden">
      ...
      <aside className="hidden md:flex md:w-72 bg-[#1C2321] text-[#E5E5E0] flex-col shadow-2xl z-20 flex-shrink-0">
        ...
        <div className="p-8 border-t border-stone-700/50 space-y-4">
          ...
          <button onClick={handleLogout} className="w-full flex items-center gap-3 text-stone-400 hover:text-red-300 transition-colors uppercase tracking-widest text-xs py-2">
                ...
                {bookings.map(b => (
                  <tr key={b.id} className="border-b border-stone-100 hover:bg-[#F9F8F6] transition-colors">
                    <td className="py-4 md:py-6 px-4">
                      <p className="font-serif text-[#1C2321]">{b.property_name}</p>
                      <p className="text-xs text-stone-400">ID: {b.id}</p>
                    </td>
                    <td className="py-4 md:py-6 px-4">
                      <p className="font-medium">{b.guest_name}</p>
                      <p className="text-sm text-stone-400">{b.email}</p>
                    </td>
                    <td className="py-4 md:py-6 px-4 text-sm font-light">{b.check_in} — {b.check_out}</td>
                    <td className="py-4 md:py-6 px-4">
                      <span className={`text-xs uppercase tracking-widest font-medium ${
                        b.status==="confirmed"?"text-[#1C2321]":b.status==="pending"?"text-[#D4AF37]":"text-stone-400"}`}>
                        {b.status}
                      </span>
                    </td>
                        ...
                        <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                              ...
                              <p className="text-stone-800 flex items-center gap-2 text-sm"><FaEnvelopeOpen className="text-stone-400 flex-shrink-0" size={12}/> {selectedClient.email}</p>
                            </div>
                            <div>
                              <p className="text-[10px] uppercase tracking-widest text-stone-400 mb-1">Phone</p>
                              <p className="text-stone-800 flex items-center gap-2 text-sm"><FaPhone className="text-stone-400" size={12}/> {selectedClient.phone||"Not provided"}</p>
                            </div>
                          </div>
                  ...
                  <tbody>
                    {customers.map(c => (
                      <tr key={c.id} className="border-b border-stone-100 hover:bg-[#F9F8F6] transition-colors">
                        <td className="py-4 md:py-6 px-4">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-[#1C2321] text-white flex items-center justify-center font-serif text-sm flex-shrink-0">
        ...
        {activeTab === "consultations" && (
          <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{duration:0.4}} className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label:"Total",     value: consultCounts.total,     color:"border-l-stone-400"  },
                { label:"Pending",   value: consultCounts.pending,   color:"border-l-amber-400"  },
                { label:"Confirmed", value: consultCounts.confirmed, color:"border-l-green-500"  },
                { label:"Completed", value: consultCounts.completed, color:"border-l-stone-300"  },
              ].map((s,i) => (
                <div key={i} className={`bg-white p-5 border border-stone-100 border-l-4 ${s.color} shadow-sm`}>
            ...
            <div className="bg-white border border-stone-100 shadow-sm overflow-hidden">
              <div className="bg-[#1C2321] text-white px-6 md:px-8 py-5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-stone-400 text-[10px] uppercase tracking-widest">All booked appointments</p>
                  </div>
                </div>
                <button onClick={fetchConsultations} className="text-stone-400 hover:text-white transition-colors" title="Refresh">
                  <FaSync />
                </button>
              </div>
              ...
              {loadingConsults ? (
                ...
                <div className="p-20 text-center">
                  <p className="text-red-500 mb-4">{consultsError}</p>
                  <button onClick={fetchConsultations} className="text-xs uppercase tracking-widest border-b border-stone-900 pb-1 hover:text-stone-600">Retry</button>
                </div>
              ) : consultations.length === 0 ? (
                <div className="p-20 text-center">
                  <FaCalendarCheck className="text-4xl text-stone-200 mx-auto mb-4"/>
                  <p className="font-serif text-stone-400 italic">No consultation requests yet.</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-left border-collapse min-w-[700px]">
                    <thead>
                      <tr className="border-b border-stone-200 bg-stone-50">
                        {["#","Client","Date & Time","Notes","Status","Actions"].map(h => (
                          <th key={h} className="py-4 px-4 md:px-6 text-[10px] uppercase tracking-widest text-stone-500 font-medium">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-stone-100">
                      {consultations.map(c => (
                        <tr key={c.id} className="hover:bg-[#F9F8F6] transition-colors group">
                          <td className="py-4 md:py-5 px-4 md:px-6 text-stone-400 text-sm font-light">#{c.id}</td>

                          <td className="py-4 md:py-5 px-4 md:px-6">
                            <p className="font-serif text-[#1C2321]">{c.user_name || `User #${c.user_id}`}</p>
                            {c.user_email && <p className="text-xs text-stone-400 mt-0.5">{c.user_email}</p>}
                          </td>

                          <td className="py-4 md:py-5 px-4 md:px-6">
                            <p className="text-sm text-stone-700">{c.date ? new Date(c.date).toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"}) : "—"}</p>
                            <p className="text-xs text-stone-400 mt-0.5">
                              {c.hour != null ? `${String(c.hour).padStart(2,"0")}:${String(c.minute||0).padStart(2,"0")}` : "Time TBD"}
                            </p>
                          </td>

                          <td className="py-4 md:py-5 px-4 md:px-6 max-w-[200px]">
                            <p className="text-sm text-stone-600 font-light truncate">{c.notes || <span className="text-stone-300 italic">No notes</span>}</p>
                          </td>
                          ...
                          <td className="py-4 md:py-5 px-4 md:px-6">
                            <div className="flex items-center gap-1">
                              {c.status !== "confirmed" && c.status !== "completed" && (
                                <button
                                  onClick={() => updateConsultStatus(c.id, "confirmed")}
                              ...
                              {c.status === "confirmed" && (
                                <button
                                  onClick={() => updateConsultStatus(c.id, "completed")}
                                  disabled={updatingConsult === c.id}
                              ...
                              {c.status !== "cancelled" && c.status !== "completed" && (
                                <button
                                  onClick={() => updateConsultStatus(c.id, "cancelled")}
                                  disabled={updatingConsult === c.id}
